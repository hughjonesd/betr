...
guess_s <- form_stage(form_page=myform, fields=list(
...
fields=list(guess=function(ftitle, val, ...) {
  if (is.null(val) || ! is.numeric(val) || abs(val - round(val)) >= 
      .Machine$double.eps || val < 1 || val > 10) {
...
```{r echo=1:2, eval=c(1,3,4), warning=FALSE}
ready(expt)
web_test(expt) # and submit a number in your browser
expt$handle_request("client-1")
expt$handle_request("client-1", list(give="1"))
```
